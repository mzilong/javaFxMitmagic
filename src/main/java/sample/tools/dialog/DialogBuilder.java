...
public class DialogBuilder {
    ...
    private Paint negativeBtnPaint;//否定按钮文字颜色，默认灰色
    private Paint positiveBtnPaint;
    ...
    public DialogBuilder setNegativeBtn(String negativeBtnText,OnClickListener negativeBtnOnclickListener, String color) {
        if (color != null&&color.length()>0) {
            this.negativeBtnPaint = Color.valueOf(color).brighter();
        }
    ...
    public DialogBuilder setNegativeBtn(String negativeBtnText,OnClickListener negativeBtnOnclickListener) {
        ...
        negativeBtn.setRipplerFill(Color.WHITE);
        if(negativeBtnPaint!=null){
    ...
    public DialogBuilder setPositiveBtn(String positiveBtnText, OnClickListener positiveBtnOnclickListener, String color) {
        if (color != null&&color.length()>0) {
            this.positiveBtnPaint = Color.valueOf(color);
        }
    ...
    public DialogBuilder setPositiveBtn(String positiveBtnText,  OnClickListener positiveBtnOnclickListener) {
        ...
        positiveBtn.setRipplerFill(Color.WHITE);
        if(positiveBtnPaint!=null){
    ...
    public DialogBuilder setHyperLink(String text) {
        ...
        hyperlink.setOnMouseClicked(event -> {
            JFXUtils.setLinkAutoAction(hyperlink);
        });
        return this;
    }
    ...
    public DialogBuilder addListener(ThemeChangeListener listener){
    ...
    public JFXAlert<String> create() {
        alert = new JFXAlert<>(window);
        alert.initModality(Modality.APPLICATION_MODAL);
        alert.setOverlayClose(true);
        alert.showingProperty().addListener((observable, oldValue, newValue) -> {
            if(themeChangeListener !=null&&newValue){
                themeChangeListener.themeChanged(alert.getDialogPane());
            }
        });
